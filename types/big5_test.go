...
func TestUtf8ToBig5(t *testing.T) {
	...
	r2 := "這是ช ช้าง~聽起來很 cool～It says from google 翻譯 that it's an elephant."
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if !bytes.Equal(big5, tt.expectedBig5) {
				t.Errorf("Utf8ToBig5: %x expected: %x", big5, tt.expectedBig5)
			}
		})
	}
	wg.Wait()
